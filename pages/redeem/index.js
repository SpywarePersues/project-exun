...
export default function index(){
    ...
    return(
        <div>
            ...
            <div className='my-10 p-10 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4'>
                {firedata.map((data) => {
                    return(
                        <div key={data.id} className="glassmorph md:mx-10 mx-4 w-11/12 rounded-lg hover:scale-105 transition-scale duration-300 my-3 p-4">
                            ...
                            <Link href={`/redeem/${data.id}`}><button className="font-Bebas text-center button mt-2 bg-gradient-to-r from-blue-500 to-pink-600 px-5 text-gray-200 w-fit mx-4 py-2 my-3 rounded-md text-xl">REDEEM</button></Link>
                        </div>
                    )
                })}
            </div>
